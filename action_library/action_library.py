...
class ActionLibrary(mc.MultiPointCommand):
    ...
    def turnRight(self):
        ...
        if self.handleTransforms("link_aruco_right_base"):
            ...
            self.move_to_pose({'rotate_mobile_base':-np.pi/2 -np.pi/12})
...
#        return True 
    def jointsClose(self, joints, target_point):
        ...
        for value in joint_positions.values():
            if np.isclose(value, target_point[i], atol=0.05):
                truth.append(True)
            else:
                truth.append(False)
            i+=1
    ...
    def moveWithCamera(self, target_point, joints, target_velocities=None, target_accelerations=None, focus="utensil_end", lookup=False):
        ...
        while not is_close:
            ...
            if self.handleTransforms(focus):
                correction = np.pi/7
                pan_point = 0 #self.base_to_tag_angle
                tilt_point = 0 #self.cam_to_tag_angle + correction
